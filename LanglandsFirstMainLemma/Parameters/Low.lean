...
private def lowUpstairsStationaryCandidate
    (epsilon₁ alpha₁ beta₁ : Kˣ) : K :=
  algebraMap F K (normUnits F K beta₁ : F) *
      algebraMap F K (lowEpsilon F K epsilon₁ : F) / (epsilon₁ : K) -
    (beta₁ : K) * algebraMap F K (normUnits F K alpha₁ : F) / (alpha₁ : K)

/-- The two summands of the upstairs candidate are integral. -/
private theorem lowUpstairsStationaryCandidate_mem_lattice
    (hres : residueDegree F K = 1) {v : ℕ}
    (epsilon₁ alpha₁ beta₁ : Kˣ)
    (hepsilon₁ : ord K (epsilon₁ : K) = ((v : ℤ) : WithTop ℤ))
    (halpha₁ : ord K (alpha₁ : K) = (0 : WithTop ℤ))
    (hbeta₁ : ord K (beta₁ : K) = (0 : WithTop ℤ)) :
    lowUpstairsStationaryCandidate F K epsilon₁ alpha₁ beta₁ ∈ lattice K 0 := by
  let eta : Fˣ := lowEpsilon F K epsilon₁
  let alpha : Fˣ := normUnits F K alpha₁
  let beta : Fˣ := normUnits F K beta₁
  let candidate := lowUpstairsStationaryCandidate F K epsilon₁ alpha₁ beta₁
  have hram : ramificationIndex F K = Module.finrank F K := by
    have hdegree := finrank_eq_ramificationIndex_mul_residueDegree F K
    rw [hres, mul_one] at hdegree
    exact hdegree.symm
...
/-- At a low stationary depth, only the trace and norm endpoints survive. -/
private theorem lowNormPolynomial_sub_trace_add_norm_mem
    ...
    (hgen : Algebra.adjoin (ringOfIntegers F)
      ({pi} : Set (ringOfIntegers K)) = ⊤)
    {m q : ℕ} (hm : 2 ≤ m) (hLow : m ≤ t + 1) (hq : m ≤ 2 * q)
    (x : K) (hx : x ∈ lattice K (q : ℤ)) :
    normPolynomialValue F K x - (trace F K x + norm F K x) ∈
      lattice F (m : ℤ) := by
  have htpos : 0 < t := by omega
  have hT : 2 ≤ t + 1 := hm.trans hLow
  ...
  have htrace := traceIdealLowerBound_of_integralGenerator
    F K ht hres pi hpi hgen
  have hintermediate : NormPolynomialIntermediateTermsVanishAt F K q m := by
    apply wild_normPolynomialIntermediateTermsVanishAt F K
      (Module.finrank F K) (t + 1) q m hp hT hchar rfl
      (by simpa [wildDifferentContribution] using htrace)
    have hfirst : Module.finrank F K * m ≤
        2 * q + (Module.finrank F K - 1) * m := by
      ...
      simpa only [Nat.add_sub_cancel_left] using
        Nat.add_le_add_right hq ((Module.finrank F K - 1) * m)
    exact hfirst.trans (Nat.add_le_add_left
      (Nat.mul_le_mul_left (Module.finrank F K - 1) hLow) (2 * q))
  exact normPolynomialValue_sub_trace_add_norm_mem F K hp.two_le hintermediate x hx

/-- Shifting the native stationary variable by the conductor gap reaches
at least the critical stationary depth, while staying in the low range. -/
private theorem lowShiftedStationaryDepth_bounds
    {m d epsilon : ℕ}
    (hF : IsStationaryConductorDecomposition m d epsilon)
    (hLow : m ≤ t + 1) :
    lowCriticalVariableDepth t ≤ d + epsilon + (t + 1 - m) ∧
      d + epsilon + (t + 1 - m) ≤ t + 1 := by
  have hdepth : d + epsilon + (t + 1 - m) = t + 1 - d := by
  ...
  have hsEq : lowCriticalVariableDepth t =
    ...
    omega
  rw [hdepth, hsEq]
  exact ⟨Nat.sub_le_sub_left hdcrit (t + 1), Nat.sub_le (t + 1) d⟩

/-- A stationary coefficient of a norm character annihilates the exact
norm polynomial once its variable lies in the critical stationary layer. -/
private theorem lowNormCharacter_normPolynomial_phase
    (ht : PrimeCyclicExtension.IsLowerBreak F K t)
    (hres : residueDegree F K = 1)
    (pi : ringOfIntegers K)
    (hpi : (ValuativeRel.valuation K).IsUniformizer (pi : K))
    (hgen : Algebra.adjoin (ringOfIntegers F)
      ({pi} : Set (ringOfIntegers K)) = ⊤)
    (psiF : LocalAddCharData F) (tau : NormCharacter F K)
    (alpha deltaF : Fˣ) (hT : 2 ≤ t + 1)
    (halphaLinear : ∀ z : lattice F (lowCriticalVariableDepth t : ℤ),
      tau.1 (positiveUnitOfLattice F
        (lowCriticalConductorDecomposition (t := t) hT).variableDepth_pos z) =
          psiF.character ((alpha : F) * (z : F) / (deltaF : F)))
    {s : ℕ} (hsCritical : lowCriticalVariableDepth t ≤ s) (hsT : s ≤ t + 1)
    (y : K) (hy : (((s : ℕ) : ℤ) : WithTop ℤ) ≤ ord K y) :
    psiF.character ((alpha : F) * normPolynomialValue F K y / (deltaF : F)) = 1 := by
  have htpos : 0 < t := by omega
  have hchar : residueCharacteristic F = Module.finrank F K :=
    residueCharacteristic_eq_degree_of_positive_isLowerBreak
      F K ht htpos pi hpi hgen
  let hTdec := lowCriticalConductorDecomposition (t := t) hT
  have hnormPolyY : normPolynomialValue F K y ∈ lattice F (s : ℤ) := by
    exact wild_norm_one_add_sub_one_mem_lattice F K ht htpos hsT hres
      hchar pi hpi hgen y hy
  let zF : lattice F (lowCriticalVariableDepth t : ℤ) :=
    ⟨normPolynomialValue F K y,
      lattice_antitone F (by exact_mod_cast hsCritical) hnormPolyY⟩
  have hzTau : psiF.character
      ((alpha : F) * (zF : F) / (deltaF : F)) = 1 := by
    rw [← halphaLinear zF]
    change tau.1 (positiveUnitOfLattice F hTdec.variableDepth_pos zF) = 1
    let yLat : lattice K (s : ℤ) := ⟨y, by simpa [mem_lattice] using hy⟩
    have hspos : 0 < s :=
      lt_of_lt_of_le hTdec.variableDepth_pos hsCritical
    let yUnit : Kˣ := positiveUnitOfLattice K hspos yLat
...
/-- Apply the critical norm character to the shifted variable and convert
its negative trace phase into the norm endpoint of the original variable. -/
private theorem lowShiftedStationaryTrace_conversion
    (ht : PrimeCyclicExtension.IsLowerBreak F K t)
    (hres : residueDegree F K = 1)
    (pi : ringOfIntegers K)
    (hpi : (ValuativeRel.valuation K).IsUniformizer (pi : K))
    (hgen : Algebra.adjoin (ringOfIntegers F)
      ({pi} : Set (ringOfIntegers K)) = ⊤)
    (psiF : LocalAddCharData F) {m d epsilon : ℕ}
    (hF : IsStationaryConductorDecomposition m d epsilon) (hLow : m ≤ t + 1)
    (tau : NormCharacter F K) (epsilon₁ alpha₁ beta₁ : Kˣ) (deltaF : Fˣ)
    ...
    (halphaLinear : ∀ z : lattice F (lowCriticalVariableDepth t : ℤ),
      tau.1 (positiveUnitOfLattice F
        (lowCriticalConductorDecomposition (t := t) hT).variableDepth_pos z) =
          psiF.character ((normUnits F K alpha₁ : F) * (z : F) / (deltaF : F)))
    (x : lattice K ((d + epsilon : ℕ) : ℤ)) :
    psiF.character
        (-((normUnits F K alpha₁ : F) *
          trace F K ((epsilon₁ : K) * (beta₁ : K) * (x : K) / (alpha₁ : K)) /
            (deltaF : F))) =
      psiF.character
        ((lowEpsilon F K epsilon₁ : F) * (normUnits F K beta₁ : F) *
          norm F K (x : K) / (deltaF : F)) := by
  let eta : Fˣ := lowEpsilon F K epsilon₁
  let alpha : Fˣ := normUnits F K alpha₁
  let beta : Fˣ := normUnits F K beta₁
  let v : ℕ := t + 1 - m
  ...
  have hy : (((q + v : ℕ) : ℤ) : WithTop ℤ) ≤ ord K y := by
    ...
    calc
      ...
      _ = ((((t + 1 - m : ℕ) : ℤ) : WithTop ℤ) +
          ord K (x : K)) := by rfl
  obtain ⟨hqvCritical, hqvT⟩ := lowShiftedStationaryDepth_bounds hF hLow
  have hzTau := lowNormCharacter_normPolynomial_phase F K ht hres pi hpi hgen
    psiF tau alpha deltaF hT halphaLinear hqvCritical hqvT y hy
  have hdiffY : normPolynomialValue F K y -
        (trace F K y + norm F K y) ∈ lattice F ((t + 1 : ℕ) : ℤ) := by
    apply lowNormPolynomial_sub_trace_add_norm_mem F K ht hres pi hpi hgen
      hT (le_refl _) (q := q + v) ?_ y (by simpa only [mem_lattice] using hy)
    have hm := hF.conductor_eq
...
/-- The explicit upstairs candidate has the same phase as the downstairs
norm polynomial on the native stationary variable layer. -/
private theorem lowUpstairsStationaryCandidate_phase
    (ht : PrimeCyclicExtension.IsLowerBreak F K t)
    (hres : residueDegree F K = 1)
    (pi : ringOfIntegers K)
    (hpi : (ValuativeRel.valuation K).IsUniformizer (pi : K))
    ...
    (x : lattice K ((d + epsilon : ℕ) : ℤ)) :
    psiK.character
        (lowUpstairsStationaryCandidate F K epsilon₁ alpha₁ beta₁ * (x : K) /
          (lowGammaK F K deltaF epsilon₁ : K)) =
      psiF.character ((normUnits F K beta₁ : F) * normPolynomialValue F K (x : K) /
        (lowGammaF F K deltaF epsilon₁ : F)) := by
  let eta : Fˣ := lowEpsilon F K epsilon₁
  let alpha : Fˣ := normUnits F K alpha₁
  let beta : Fˣ := normUnits F K beta₁
  let candidate := lowUpstairsStationaryCandidate F K epsilon₁ alpha₁ beta₁
  let v : ℕ := t + 1 - m
  let y : K := (epsilon₁ : K) * (beta₁ : K) * (x : K) / (alpha₁ : K)
  have hbeta : ord F (beta : F) = (0 : WithTop ℤ) := by
    simp only [beta, coe_normUnits, ord_norm, hres, one_nsmul, hbeta₁]
  have heta : ord F (eta : F) = (((v : ℕ) : ℤ) : WithTop ℤ) :=
    lowEpsilon_order F K hres epsilon₁ hepsilon₁
  have hdiffX := lowNormPolynomial_sub_trace_add_norm_mem F K ht hres pi hpi hgen
    hF.conductor_gt_one hLow hF.conductor_le_two_variableDepth (x : K) x.property
  have hconversion := lowShiftedStationaryTrace_conversion F K ht hres pi hpi hgen
    psiF hF hLow tau epsilon₁ alpha₁ beta₁ deltaF hdelta hepsilon₁
    halpha₁ hbeta₁ hT halphaLinear x
  have hmv : m + v = t + 1 := by
    dsimp only [v]
  ...
  have hargCandidate : candidate * (x : K) /
    ...
    dsimp only [candidate, lowUpstairsStationaryCandidate, y, lowGammaK, alpha, beta, eta]
...
/-- Pointwise agreement with the downstairs norm-polynomial phase identifies
an integral upstairs numerator with the stationary coefficient class. -/
private theorem lowStationaryClass_of_normPolynomial_phase
    (chiF : LocalQuasiCharData F) (chiK : LocalQuasiCharData K)
    (psiF : LocalAddCharData F) (psiK : LocalAddCharData K)
    ...
    (hc : latticeQuotientMk F (by omega) c =
      stationaryCoefficientClass F chiF psiF h.targetDecomposition
        gammaF hgammaF)
    (hphase : ∀ x : lattice K ((dK + epsilonK : ℕ) : ℤ),
      psiK.character ((b : K) * (x : K) / (gammaK : K)) =
        psiF.character ((c : F) * normPolynomialValue F K (x : K) /
          (gammaF : F))) :
    latticeQuotientMk K (by omega) b =
      stationaryCoefficientClass K chiK psiK h.sourceDecomposition
        gammaK hgammaK := by
  rw [stationaryClass_compNorm F K chiF chiK psiF psiK h hchi hpsi
    gammaF gammaK hgammaF hgammaK, ← hc]
  ...
  obtain ⟨x, rfl⟩ := latticeQuotientMk_surjective K
    (Int.ofNat_le.mpr h.sourceDecomposition.variableDepth_le_conductor) z
  ...
  have hadjoint := normPolynomialAdjoint_pairing F K h psiF psiK
    gammaF gammaK hgammaF hgammaK
    (latticeQuotientMk F (by omega) c)
    (latticeQuotientMk K
      (Int.ofNat_le.mpr h.sourceDecomposition.variableDepth_le_conductor) x)
  rw [normPolynomial_mk, stationaryPairingLeft_mk_mk] at hadjoint
  rw [← hadjoint, coe_normPolynomialRepresentative]
  exact congrArg Units.val (hphase x)
...
theorem lowUpstairsStationaryClass
  ...
  have halpha : ord F (alpha : F) = (0 : WithTop ℤ) := by
    simp only [alpha, coe_normUnits, ord_norm, hres, one_nsmul, halpha₁]
  have hcandidate := lowUpstairsStationaryCandidate_mem_lattice F K hres
    epsilon₁ alpha₁ beta₁ hepsilon₁ halpha₁ hbeta₁
  refine ⟨hcandidate, ?_⟩
  let tauData : LocalQuasiCharData F :=
    quasiCharDataOfIsConductor F tau.1 (t + 1)
      (ramifiedNormCharacter_conductor
  ...
  apply lowStationaryClass_of_normPolynomial_phase F K chiF chiK psiF psiK
    ...
    ⟨candidate, hcandidate⟩ hbetaClass
  intro x
  exact lowUpstairsStationaryCandidate_phase F K ht hres pi hpi hgen
    psiF psiK hpsi hF hLow tau epsilon₁ alpha₁ beta₁ deltaF hdelta hepsilon₁
    halpha₁ hbeta₁ hT halphaLinear x
