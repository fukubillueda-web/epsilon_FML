...
omit ht hres pi hpi hgen in
private def highParameterIntermediateUpstairsCandidate
    (alpha1 beta1 : Kˣ)
    (hcandidate :
...
omit ht hres pi hpi hgen in
private def highParameterIntermediateBetaNorm
    (beta1 : Kˣ)
    (hbetaNLat : norm F K (beta1 : K) ∈ lattice F 0) :
    lattice F 0 :=
  ⟨norm F K (beta1 : K), hbetaNLat⟩

omit [Module.Finite F K] [PrimeCyclicExtension F K]
  ht hres pi hpi hgen in
private theorem highParameterIntermediate_upstairsCandidate_mem
    {v d : ℕ}
    (alpha : Fˣ) (alpha1 : Kˣ)
  ...
  simpa only [alphaN, betaN] using (lattice K 0).sub_mem hbetaMapLat hsecond

omit [Module.Finite F K] [PrimeCyclicExtension F K]
  ht hres pi hpi hgen in
private theorem highParameterIntermediate_betaNorm_class
    {d epsilon : ℕ}
    (chiF : LocalQuasiCharData F) (psiF : LocalAddCharData F)
    (hF : IsStationaryConductorDecomposition chiF.conductor d epsilon)
    (gammaF : Fˣ)
    ...
    latticeQuotientMk F (Int.natCast_nonneg d)
        (highParameterIntermediateBetaNorm F K beta1
          hbeta1.norm_exactDepth.1) =
      stationaryCoefficientClass F chiF psiF hF gammaF hgammaF := by
  calc
    latticeQuotientMk F (Int.natCast_nonneg d)
        (highParameterIntermediateBetaNorm F K beta1
          hbeta1.norm_exactDepth.1) =
        latticeQuotientMk F (Int.natCast_nonneg d) b := by
      apply (latticeQuotientMk_eq_mk_iff F (Int.natCast_nonneg d)).2
      change norm F K (beta1 : K) - (b : F) ∈ lattice F (d : ℤ)
      have hneg := neg_mem_lattice F hbeta1.norm_congruent
      rw [hbcoe]
      simpa using hneg
...
omit ht hres pi hpi hgen in
private theorem highParameterIntermediate_normPolynomial_character
    {dK epsilonK : ℕ}
    (chiF : LocalQuasiCharData F) (psiF : LocalAddCharData F)
    ...
    (hgammaF : ord F (gammaF : F) =
      (((chiF.conductor : ℤ) + psiF.conductor : ℤ) : WithTop ℤ))
    (betaN : F) (hbetaNLat : betaN ∈ lattice F 0)
...
private theorem highParameterIntermediate_candidate_character
    {v d dK epsilonK : ℕ}
    (psiF : LocalAddCharData F)
    ...
    psiF.character
        (norm F K (beta1 : K) *
          (trace F K (x : K) + norm F K (x : K)) / (gammaF : F)) =
      psiF.character
        (trace F K
          ((algebraMap F K (norm F K (beta1 : K)) -
              (beta1 : K) * algebraMap F K (norm F K (alpha1 : K)) /
                (alpha1 : K)) * (x : K) /
            ((Units.map (algebraMap F K) gammaF : Kˣ) : K))) := by
...
omit ht hres pi hpi hgen in
private theorem highParameterIntermediate_candidate_eq_adjoint
    ...
    (hcandidate :
      algebraMap F K (norm F K (beta1 : K)) -
          (beta1 : K) * algebraMap F K (norm F K (alpha1 : K)) /
            (alpha1 : K) ∈ lattice K 0) :
    latticeQuotientMk K (Int.natCast_nonneg dK)
        (highParameterIntermediateUpstairsCandidate F K alpha1 beta1
          hcandidate) =
      normPolynomialAdjoint F K h psiF psiK gammaF
        (Units.map (algebraMap F K) gammaF) hgammaF hgammaK
        (latticeQuotientMk F (Int.natCast_nonneg d)
          (highParameterIntermediateBetaNorm F K beta1
            hbeta1.norm_exactDepth.1)) := by
  let gammaK : Kˣ := Units.map (algebraMap F K) gammaF
  let candidate : lattice K 0 :=
    highParameterIntermediateUpstairsCandidate F K alpha1 beta1 hcandidate
  let betaNorm : lattice F 0 :=
    highParameterIntermediateBetaNorm F K beta1 hbeta1.norm_exactDepth.1
  ...
  change psiF.character
    ...
    psiF.character
      (norm F K (beta1 : K) *
        (normPolynomialRepresentative F K h x : F) / (gammaF : F))
  symm
  rw [coe_normPolynomialRepresentative]
  exact (highParameterIntermediate_normPolynomial_character F K chiF psiF
    hterms gammaF hgammaF (norm F K (beta1 : K))
    hbeta1.norm_exactDepth.1 x).trans (by
      simpa only [candidate, gammaK,
        highParameterIntermediateUpstairsCandidate] using
        highParameterIntermediate_candidate_character F K psiF hshift
          gammaF alpha alpha1 halpha1 beta beta1 hbeta1 hconversion x)
...
theorem highParameter_intermediate_upstairsClass_core
  ...
  let gammaK : Kˣ := Units.map (algebraMap F K) gammaF
  have hcandidate := highParameterIntermediate_upstairsCandidate_mem
    F K alpha alpha1 halpha1 beta beta1 hbeta1
  have h0d : (0 : ℤ) ≤ (d : ℤ) := Int.natCast_nonneg d
  ...
  have hbetaNormClass : latticeQuotientMk F h0d betaNorm =
      stationaryCoefficientClass F chiF psiF h.targetDecomposition
        gammaF hgammaF :=
    highParameterIntermediate_betaNorm_class F K chiF psiF
      h.targetDecomposition gammaF hgammaF beta beta1 hbeta1 b hbcoe hbclass
  have hadjoint :
      latticeQuotientMk K h0dK candidate =
        normPolynomialAdjoint F K h psiF psiK gammaF gammaK
          hgammaF hgammaK (latticeQuotientMk F h0d betaNorm) :=
    highParameterIntermediate_candidate_eq_adjoint F K chiF chiK psiF psiK
      h hpsi hterms hshift gammaF hgammaF hgammaK alpha alpha1 halpha1
      beta beta1 hbeta1 hconversion hcandidate
  have hadjointClass :
      latticeQuotientMk K h0dK candidate =
        normPolynomialAdjoint F K h psiF psiK gammaF gammaK
          hgammaF hgammaK
  ...
  refine ⟨?_, ?_⟩
  · simpa only [candidate, gammaK,
      highParameterIntermediateUpstairsCandidate] using hadjointClass
  · calc
      latticeQuotientMk K h0dK candidate =
          normPolynomialAdjoint F K h psiF psiK gammaF gammaK
            hgammaF hgammaK
