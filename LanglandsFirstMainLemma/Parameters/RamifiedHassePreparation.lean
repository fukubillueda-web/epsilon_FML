...
/-- The tame power relation supplies the affine characteristic-two finite data. -/
private theorem RamifiedHasseCharTwoFiniteData.exists_of_powerRelation
    {k : Type u} [Field k] [Fintype k] [CharP k 2]
    {ell : ℕ} [Fact ell.Prime] {theta : FiniteAddChar k}
    {epsilon c : k} (lower : HasseFunction k theta) {upper : k → ℂ}
    (htheta : theta ≠ 1) (hell : Odd ell) (hc : c ≠ 0)
    (hepsilon : epsilon = (ell : k) * c ^ 2)
    (hdiv : ell ∣ Fintype.card k - 1)
    (hpower : ∀ x, upper (c⁻¹ * x) = lower x ^ ell) :
    ∃ D : RamifiedHasseCharTwoFiniteData k ell epsilon (fun x ↦ lower x) upper,
      D.c = c := by
  obtain ⟨scale, hscale, q, lambda, hlower⟩ :=
    exists_charTwo_affine_normalForm htheta lower
  letI : Algebra (ZMod 2) k := ZMod.algebra k 2
  let f := Module.finrank (ZMod 2) k
  have hcard : Fintype.card k = 2 ^ f := by
    simpa only [ZMod.card] using
      (Module.card_eq_pow_finrank (K := ZMod 2) (V := k))
  have hparity : ell % 8 = 3 ∨ ell % 8 = 5 → Even f := by
    intro hmod
    apply charTwo_difficultParity (by
      obtain ⟨r, hr⟩ := hell
      omega) (by rwa [← hcard]) hmod
  let D : RamifiedHasseCharTwoFiniteData k ell epsilon (fun x ↦ lower x) upper :=
    { degree_odd := hell
      q := q
      c := c
      c_ne_zero := hc
      ...
      traceOne := absoluteTraceChar_one k
      difficultParity := hparity
      upper_binomial_pointwise := fun x ↦ by
        calc
          upper ((c * scale)⁻¹ * x) = upper (c⁻¹ * (scale⁻¹ * x)) := by
            congr 1
            rw [mul_inv_rev]
            ring
          _ = lower (scale⁻¹ * x) ^ ell := hpower (scale⁻¹ * x)
          _ = q.affine lambda x ^ ell := by rw [hlower]
          _ = q.affine lambda ((ell : k) * x) *
              absoluteTraceChar k (-((ell.choose 2 : k) * x ^ 2)) :=
            (ramifiedHasseCharTwo_binomialCorrection q lambda ell x).symm }
  exact ⟨D, rfl⟩
...
/-- In the tame case the normalized trace is the degree-scaled source coordinate. -/
private theorem tame_traceCoordinate_eq
    (P : PrimeCyclicPreparation F K) {d dK : ℕ}
    (G : RamifiedHasseLocalGeometryData F K P d dK)
    (ht0 : P.t = 0) (x : ResidueField F) :
...
/-- Positive ramification break forces the residual trace coordinate to vanish. -/
private theorem wild_traceCoordinate_eq_zero
    (P : PrimeCyclicPreparation F K) {d dK : ℕ}
    (G : RamifiedHasseLocalGeometryData F K P d dK)
    (htpos : 0 < P.t) (x : ResidueField F) :
...
/-- Newton's identity in the source coordinates, before reduction to the residue field. -/
private theorem ramifiedHasseSecondCoordinateValue_newton
    (P : PrimeCyclicPreparation F K) {d dK : ℕ}
    (G : RamifiedHasseLocalGeometryData F K P d dK)
    (a : F) :
    (2 : F) * ramifiedHasseSecondCoordinateValue F K P d dK a =
      ramifiedHasseTraceCoordinateValue F K P d dK a ^ 2 -
        ramifiedHasseNormalizedTrace F K P d dK * a ^ 2 := by
...
/-- Reducing Newton's identity gives the exact wild quadratic coefficient. -/
private theorem wild_secondCoordinate_eq
    (P : PrimeCyclicPreparation F K) {d dK : ℕ}
    (G : RamifiedHasseLocalGeometryData F K P d dK)
    (htpos : 0 < P.t) (x : ResidueField F) :
    -ramifiedHasseSecondCoordinate F K P G x =
  ...
  have he2 : e2 ∈ lattice F 0 :=
    ramifiedHasseSecondCoordinateValue_mem F K P G a ha
  have hfield := ramifiedHasseSecondCoordinateValue_newton F K P G a
  ...
  rw [hzred, he2red, hepsilonred, hared,
    wild_traceCoordinate_eq_zero F K P G htpos x] at hres
...
private theorem ramifiedHasseFiniteComparison
  ...
  · have hc0 := G.tameCoordinate_ne_zero ht0
    have htrace := tame_traceCoordinate_eq F K P G ht0
    ...
    · letI : CharP (ResidueField F) 2 := ringChar.of_eq hchar2
      obtain ⟨D, hD⟩ :=
        RamifiedHasseCharTwoFiniteData.exists_of_powerRelation lower.function
          (upper := fun x ↦ upper.function
            (PhaseReductionResidualCoordinateSource.residueEquiv P.hres x))
          htheta G.degree_odd hc0 hepsilon hdiv hpower
      exact .charTwo ht0 D hD
  ...
  · have hchar : residueCharacteristic F = Module.finrank F K :=
    ...
    have hepsilon :=
      RamifiedHasseLocalGeometryData.wild_epsilonResidue_eq
        (F := F) (K := K) G htpos
    have htrace := wild_traceCoordinate_eq_zero F K P G htpos
    have hsecond := wild_secondCoordinate_eq F K P G htpos
