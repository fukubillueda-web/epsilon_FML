...
/-- Boundary conductor arithmetic identifies the two critical depths. -/
private theorem lowSourceBoundary_depths
    {m : ℕ} (h : IsStationaryConductorDecomposition m d epsilon)
    (hepsilon : epsilon = 1) (hboundary : m = t + 1) :
    0 < t ∧ d < t ∧ Odd (t + 1) ∧ lowCriticalFloorDepth t = d := by
  have htEq : t = 2 * d := by
    have hm := h.conductor_eq
  ...
  have hdFloor : lowCriticalFloorDepth t = d := by
    ...
    omega
  exact ⟨htpos, hd, hoddT, hdFloor⟩

/-- Choose the source unit and transport the displayed upstairs value to raw factors. -/
private theorem lowSourceDisplayed_exactCriticalTransport_raw
    (WUp : LowOddUpstairsWitness F K ht hres pi hpi hgen data chiK psiK hF
      hminimal hchi hpsi hLow delta epsilon1 hdelta hepsilon1 hT hgammaF
        hgammaK P table)
    (hepsilon : epsilon = 1)
    ...
    (C : FrobeniusResidualAddCharData F (Module.finrank F K))
    (X : ResidueField F) (upperUxUnit : Kˣ)
    (hUpperUxUnit :
      let z := (C.frobeniusEquiv hchar).symm X
      let x := phaseReductionUpperSourceDisplacement F K pi d z
      (upperUxUnit : K) = 1 + lowNormalizedRatio F K epsilon1 P * x) :
    let z := (C.frobeniusEquiv hchar).symm X
    let x := phaseReductionUpperSourceDisplacement F K pi d z
    let A := lowOddExactCoefficient (F := F) (K := K) (P := P) (hT := hT)
    let u := lowNormalizedRatio F K epsilon1 P
    let n := lowOddNormalizedNorm (F := F) (K := K) (P := P) (hT := hT)
    ∃ upperUnit : Kˣ, (upperUnit : K) = 1 + x ∧
      lowSourceDisplayedUpstairsCriticalPolarFunction F K ht hres pi hpi hgen
          data chiK psiK hF hminimal hchi hpsi hLow delta epsilon1 hdelta
            hepsilon1 hT hgammaF hgammaK P table (Module.finrank F K) hchar C
              WUp X =
  ...
  have hraw := lowOdd_actualStationaryRatios_exactCriticalTransport
    F K ht hres pi hpi hgen data chiK psiK hF hminimal hchi hpsi hLow
      delta epsilon1 hdelta hepsilon1 hT hgammaF hgammaK P table WUp x
        upperUnit upperUxUnit hUpperUnit hUpperUxUnit
  refine ⟨upperUnit, hUpperUnit, ?_⟩
  rw [lowSourceDisplayedUpstairsFunction_displacement_apply_eq_raw
    F K ht hres pi hpi hgen data chiK psiK hF hminimal hchi hpsi hLow
      delta epsilon1 hdelta hepsilon1 hT hgammaF hgammaK P table WUp
        hepsilon (Module.finrank F K) hchar C X]
  have hrawC := congrArg (Units.val : ℂˣ → ℂ) hraw
...
/-- Evaluate the base row at the norm of the displayed source displacement. -/
private theorem lowSourceBoundary_baseFunction_eq_raw
    (hepsilon : epsilon = 1) (htpos : 0 < t) (hd : d < t)
    (hchar : residueCharacteristic F = Module.finrank F K)
    (C : FrobeniusResidualAddCharData F (Module.finrank F K))
    (X : ResidueField F) (upperUnit : Kˣ)
    (hUpperUnit :
      let z := (C.frobeniusEquiv hchar).symm X
      (upperUnit : K) =
        1 + phaseReductionUpperSourceDisplacement F K pi d z) :
    let z := (C.frobeniusEquiv hchar).symm X
    let x := phaseReductionUpperSourceDisplacement F K pi d z
    lowSourceBaseFunction F K ht hres pi hpi hgen data chiK psiK hF
        hminimal hchi hpsi hLow delta epsilon1 hdelta hepsilon1 hT hgammaF
      ...
      (data.baseAddChar.character
        (((P.beta : F) / (lowGammaF F K delta epsilon1 : F)) *
          phaseReductionNormPolynomial F K x) : ℂ) *
        ((data.twistData 1).character (normUnits F K upperUnit) : ℂ)⁻¹ := by
  ...
  have hbaseClass : phaseReductionResidualClass F S.lowerUniformizer
    ...
    exact hclass'.trans hzpow
  dsimp only at hbaseEval
  change lowSourceBaseFunction F K ht hres pi hpi hgen data chiK psiK hF
    hminimal hchi hpsi hLow delta epsilon1 hdelta hepsilon1 hT hgammaF
      hgammaK P table
        (phaseReductionResidualClass F S.lowerUniformizer S.lower_order d yBase) = _
    at hbaseEval
  simpa only [hbaseClass, baseUnit] using hbaseEval

/-- Evaluate the boundary norm row at the norm-scaled displayed coordinate. -/
private theorem lowSourceBoundary_normFunction_eq_raw
    (hboundary : (data.twistData 1).conductor = t + 1)
    (htpos : 0 < t) (hd : d < t) (hoddT : Odd (t + 1))
    (hdFloor : lowCriticalFloorDepth t = d)
    (hchar : residueCharacteristic F = Module.finrank F K)
    (C : FrobeniusResidualAddCharData F (Module.finrank F K))
    (X : ResidueField F) (upperUxUnit : Kˣ)
    (hUpperUxUnit :
    ...
    let jOne : OddNormIndex F K := ⟨1, one_ne_zero⟩
    lowSourceNormFunction F K ht hres pi hpi hgen data hF delta epsilon1
        hdelta hT hgammaF P jOne (residueMap F nO * X) =
      (data.baseAddChar.character
        (((P.alpha : F) / (delta : F)) *
          phaseReductionNormPolynomial F K (u * x)) : ℂ) *
        ((lowNormCharacterGenerator F K ht hres pi hpi hgen).1
          (normUnits F K upperUxUnit) : ℂ)⁻¹ := by
  dsimp only at hUpperUxUnit ⊢
  let z := (C.frobeniusEquiv hchar).symm X
  let x := phaseReductionUpperSourceDisplacement F K pi d z
  let u := lowNormalizedRatio F K epsilon1 P
  ...
  have htauClass : phaseReductionResidualClass F S.lowerUniformizer
    ...
    exact hclass'.trans (by simp only [tauX, hzpow])
  ...
  simp only [hdFloor] at htauEval
  change lowSourceNormFunction F K ht hres pi hpi hgen data hF delta epsilon1
    hdelta hT hgammaF P ⟨1, one_ne_zero⟩
      (phaseReductionResidualClass F S.lowerUniformizer S.lower_order d yTau) = _
    at htauEval
  simpa only [htauClass, tauX, tauUnit] using htauEval
      ...
      lowSourceBaseFunction F K ht hres pi hpi hgen data chiK psiK hF
        ...
        (data.baseAddChar.character
          (A * (phaseReductionNormHigherPart F K (u * x) -
            n * phaseReductionNormHigherPart F K x)) : ℂ) := by
  dsimp only at hUpperUxUnit ⊢
  obtain ⟨htpos, hd, hoddT, hdFloor⟩ :=
    lowSourceBoundary_depths hF hepsilon hboundary
  let hchar : residueCharacteristic F = Module.finrank F K :=
    residueCharacteristic_eq_degree_of_positive_break F K ht htpos pi hpi
      hgen
  obtain ⟨upperUnit, hUpperUnit, htransport⟩ :=
    lowSourceDisplayed_exactCriticalTransport_raw F K ht hres pi hpi hgen
      data chiK psiK hF hminimal hchi hpsi hLow delta epsilon1 hdelta
        hepsilon1 hT hgammaF hgammaK P table WUp hepsilon hchar C X
          upperUxUnit hUpperUxUnit
  have hbase := lowSourceBoundary_baseFunction_eq_raw F K ht hres pi hpi
    hgen data chiK psiK hF hminimal hchi hpsi hLow delta epsilon1 hdelta
      hepsilon1 hT hgammaF hgammaK P table hepsilon htpos hd hchar C X
        upperUnit hUpperUnit
  have htau := lowSourceBoundary_normFunction_eq_raw F K ht hres pi hpi
    hgen data hF delta epsilon1 hdelta hepsilon1 hT hgammaF P hboundary
      htpos hd hoddT hdFloor hchar C X upperUxUnit hUpperUxUnit
  dsimp only at hbase htau htransport
  rw [← hbase, ← htau] at htransport
  exact htransport
