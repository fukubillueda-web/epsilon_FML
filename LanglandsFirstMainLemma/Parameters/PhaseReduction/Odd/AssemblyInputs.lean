...
/-! ## Coherent high-source construction helpers -/

/-- Promote an order-zero field element to the depth-zero unit filtration. -/
private def highOddCoherentUnitAtDepthZero
    (E : Type*) [Field E] [ValuativeRel E] [TopologicalSpace E]
    [IsNonarchimedeanLocalField E]
    (x : E) (hx : ord E x = 0) : unitFiltration E 0 := by
  have hx0 : x ≠ 0 := by
    apply (ord_ne_top_iff E).1
    rw [hx]
    simp
  let u : Eˣ := Units.mk0 x hx0
  exact ⟨u, (mem_unitFiltration_zero E u).2 (by
    simpa only [u, Units.val_mk0] using hx)⟩

@[simp]
private theorem highOddCoherentUnitAtDepthZero_coe
    (E : Type*) [Field E] [ValuativeRel E] [TopologicalSpace E]
    [IsNonarchimedeanLocalField E]
    (x : E) (hx : ord E x = 0) :
    (((highOddCoherentUnitAtDepthZero E x hx : unitFiltration E 0) : Eˣ) : E) =
      x := by
  rfl

/-- Pointwise depth congruences multiply to a congruence of the corresponding
depth-zero unit products. -/
private theorem highOddCoherent_product_congruent_of_pointwise
    (E : Type*) [Field E] [ValuativeRel E] [TopologicalSpace E]
    [IsNonarchimedeanLocalField E]
    {iota : Type*} [Fintype iota]
    (d : ℕ) (left right : iota → unitFiltration E 0)
    (hpointwise : ∀ i,
      CongruentAtDepth (d : ℤ)
        (((left i : unitFiltration E 0) : Eˣ) : E)
        (((right i : unitFiltration E 0) : Eˣ) : E)) :
    CongruentAtDepth (d : ℤ)
      (((∏ i, left i : unitFiltration E 0) : Eˣ) : E)
      (((∏ i, right i : unitFiltration E 0) : Eˣ) : E) := by
  apply (unitFiltrationQuotientMk_eq_mk_iff_congruentAtDepth E
    (Nat.zero_le d) _ _).1
  rw [map_prod, map_prod]
  apply Finset.prod_congr rfl
  intro i _
  exact (unitFiltrationQuotientMk_eq_mk_iff_congruentAtDepth E
    (Nat.zero_le d) _ _).2 (hpointwise i)

/-- Turn the exact field-level norm/product error into the congruence needed
for the quotient-level stationary norm product. -/
private theorem highOddCoherent_norm_congruent_to_product
    (F K : Type*)
    [Field F] [ValuativeRel F] [TopologicalSpace F]
    [IsNonarchimedeanLocalField F]
    [Field K] [ValuativeRel K] [TopologicalSpace K]
    ...
    [Module.Free F K] [Module.Finite F K]
    {iota : Type*} [Fintype iota]
    (d : ℕ) (upstairs : unitFiltration K 0)
    (linear : iota → F) (linearFactors : iota → unitFiltration F 0)
    (hlinearCoe : ∀ i,
      (((linearFactors i : unitFiltration F 0) : Fˣ) : F) = linear i)
    (hnormProduct :
      norm F K (((upstairs : unitFiltration K 0) : Kˣ) : K) -
        ∏ i, linear i ∈ lattice F (d : ℤ)) :
    CongruentAtDepth (d : ℤ)
      ((normUnits F K (upstairs : Kˣ) : Fˣ) : F)
      (((∏ i, linearFactors i : unitFiltration F 0) : Fˣ) : F) := by
  rw [congruentAtDepth_iff_sub_mem_lattice]
  have hlinearFactorsCoe :
      (((∏ i, linearFactors i : unitFiltration F 0) : Fˣ) : F) =
        ∏ i, linear i := by
    change (Units.coeHom F)
      ((unitFiltration F 0).subtype (∏ i, linearFactors i)) = _
    rw [map_prod, map_prod]
    apply Finset.prod_congr rfl
    intro i _
    exact hlinearCoe i
  rw [hlinearFactorsCoe]
  simpa only [coe_normUnits] using hnormProduct

set_option maxHeartbeats 16000000 in
/-- Build the coherent high source from the single dependent parameter tuple
supplied by `HighOddIntermediateParameterData`. -/
private theorem highOddCoherentPhaseSource_of_parameter_data
    (F K : Type)
    [Field F] [ValuativeRel F] [TopologicalSpace F]
    [IsNonarchimedeanLocalField F]
    [Field K] [ValuativeRel K] [TopologicalSpace K]
  ...
  let upstairs : unitFiltration K 0 :=
    highOddCoherentUnitAtDepthZero K candidate hcandidateOrd
  ...
  let factors : ZMod p → unitFiltration F 0 := fun j ↦
    highOddCoherentUnitAtDepthZero F (lower j) (hlowerOrd j)
  let linearFactors : ZMod p → unitFiltration F 0 := fun j ↦
    highOddCoherentUnitAtDepthZero F (linear j) (hlinearOrd j)
  have hp : p.Prime := by
  ...
  have hcongruentLinear : CongruentAtDepth (d : ℤ)
    ...
    apply highOddCoherent_norm_congruent_to_product F K d upstairs linear
      linearFactors
    · intro j
      simp only [linearFactors, highOddCoherentUnitAtDepthZero_coe]
    · simpa only [upstairs, highOddCoherentUnitAtDepthZero_coe] using
        hnormProduct
  ...
  have hfactorProductCongruent : CongruentAtDepth (d : ℤ)
      ...
      (((∏ j, linearFactors j : unitFiltration F 0) : Fˣ) : F) := by
    apply highOddCoherent_product_congruent_of_pointwise F d factors
      linearFactors
    intro j
    simpa only [factors, linearFactors,
      highOddCoherentUnitAtDepthZero_coe] using hfactorCongruent j
  ...
  have hclosed : CongruentAtDepth (d : ℤ)
      ...
      (norm F K (beta1 : K) ^ p -
        norm F K (beta1 : K) * norm F K (alpha1 : K) ^ (p - 1)) := by
    rw [congruentAtDepth_iff_sub_mem_lattice]
    simpa only [upstairs, highOddCoherentUnitAtDepthZero_coe, coe_normUnits,
  ...
  let Q : OddIntermediateHighProductData F K ht hres pi hpi hgen
    ...
    · rfl
    · simpa only [product, upstairs, highOddCoherentUnitAtDepthZero_coe,
        candidateLattice, candidate] using hcandidateClass
    · intro j
      rfl
    · intro j
      simpa only [product, factors, highOddCoherentUnitAtDepthZero_coe,
        lower, lam] using hfactorCongruent j
    · intro j
      simpa only [product, factors, highOddCoherentUnitAtDepthZero_coe] using
        hlowerClass j
...
theorem highOddCoherentPhaseSource_nonempty
    ...
    Nonempty (HighOddCoherentPhaseSource F K ht hres pi hpi hgen chiF chiK
      psiF psiK hF hK hminimal hchi hpsi hodd htpos hlower hupper gammaF
        hgammaF) :=
  highOddCoherentPhaseSource_of_parameter_data F K ht hres pi hpi hgen
    chiF chiK psiF psiK hF hK hminimal hchi hpsi hodd htpos hlower hupper
      gammaF hgammaF H
