...
section LowGreenBoundary

variable {s : ℕ}
  (ht : PrimeCyclicExtension.IsLowerBreak F K (s + 1))
  (L : WildOddLowPhaseProvenance F K ht hres pi hpi hgen data phases hF)
...
private def lowGreen_boundaryNormalization :
    WildOddUpperPolarNormalization (ResidueField F) (Module.finrank F K) :=
  lowActualNormalization F K ht hres pi hpi hgen data hF L.hLow
    L.delta L.epsilon1 L.hdelta L.hepsilon1 L.hT L.hgammaF L.P hepsilon C

private def lowGreen_boundaryGenerator :
    WildOddNamedCoefficientPair (ResidueField F) :=
  wildOddLowGeneratorNamedPair F K ht hres pi hpi hgen data hF L.delta
    L.epsilon1 L.hdelta L.hT L.hgammaF L.P hparity C.lower C.lower_ne_one
      hcharOdd

private def lowGreen_boundaryBase :
    WildOddNamedCoefficientPair (ResidueField F) :=
  wildOddLowBoundaryBaseNamedPair F K ht hres pi hpi hgen data L.chiK
    L.psiK hF L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1 L.hdelta
      L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table hboundary hparity
        C.lower C.lower_ne_one hcharOdd

local notation "p" => Module.finrank F K
local notation "N" => lowGreen_boundaryNormalization
  (ht := ht) (hres := hres) (pi := pi) (hpi := hpi) (hgen := hgen)
  (hF := hF) (L := L) hepsilon C
local notation "generator" => lowGreen_boundaryGenerator
  (ht := ht) (hres := hres) (pi := pi) (hpi := hpi) (hgen := hgen)
  (hF := hF) (L := L) hparity C hcharOdd
local notation "base" => lowGreen_boundaryBase
  (ht := ht) (hres := hres) (pi := pi) (hpi := hpi) (hgen := hgen)
  (hF := hF) (L := L) hboundary hparity C hcharOdd

omit [CharP (ResidueField F) p] in
/-- Identify the two affine inputs of the actual upstairs boundary formula. -/
private theorem lowGreen_boundaryAffineCoefficients :
    (lowSourceBaseCriticalPolarFunction F K ht hres pi hpi hgen data
      L.chiK L.psiK hF L.hminimal L.hchi L.hpsi L.hLow L.delta
        L.epsilon1 L.hdelta L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P
          L.table p C).affineCoefficient hcharOdd C.lower_ne_one =
    ...
    (lowSourceNormCriticalPolarFunction F K ht hres pi hpi hgen data hF
      L.delta L.epsilon1 L.hdelta L.hT L.hgammaF L.P p C
        ⟨1, one_ne_zero⟩).affineCoefficient hcharOdd C.lower_ne_one =
        (N).etaZero * (generator).gamma := by
  have hGeneratorEta : (generator).eta = (N).etaZero := by
    simpa only [lowGreen_boundaryGenerator, lowGreen_boundaryNormalization] using
  ...
  have hBaseEta : (base).eta = (N).eta := by
    simpa only [lowGreen_boundaryBase, lowGreen_boundaryNormalization] using
  ...
  · let jOne : OddNormIndex F K := ⟨1, one_ne_zero⟩
    ...
    rw [hPair]
    simp only [WildOddCoefficientPair.translateAffine, add_zero,
      wildOddTauPowerPair, jOne, ZMod.val_one, Nat.cast_one, one_mul]
    change (generator).eta * (generator).gamma =
      (N).etaZero * (generator).gamma
    rw [hGeneratorEta]

omit [CharP (ResidueField F) p] in
/-- Evaluate the upstairs factor using the actual normalized boundary coefficients. -/
private theorem lowGreen_boundaryUpperFactor
    (hchar : residueCharacteristic F = p) :
    (lowSourceUpstairsPhase F K ht hres pi hpi hgen data L.chiK L.psiK
      hF L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1 L.hdelta
        L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table L.WUp
          ).criticalFactor =
      wildOddUpperPairPhase C.lower
        (wildOddUpperPairBoundary p (N).etaZero (N).dZero (base).gamma
          (generator).gamma) := by
  obtain ⟨hbaseCoeff, hnormCoeff⟩ := lowGreen_boundaryAffineCoefficients
    (ht := ht) (hres := hres) (pi := pi) (hpi := hpi) (hgen := hgen)
    (hF := hF) (L := L) hboundary hepsilon hparity C hcharOdd
  let units : ResidueField F → Kˣ := fun X => lowActualSourceUnit F K ht
    hres pi hpi hgen data hF L.hLow L.delta L.epsilon1 L.hdelta
      L.hepsilon1 L.hT L.hgammaF L.P hepsilon C X
  have hunits := lowActualSourceUnit_coe F K ht hres pi hpi hgen data hF
  ...
  have hPair :
    ...
    dsimp only [lowGreen_boundaryNormalization] at hbaseCoeff' hnormCoeff' ⊢
  ...
  exact lowGreen_upperFactor F K p hchar hres C hcharOdd
    (lowSourceUpstairsPhase F K ht hres pi hpi hgen data L.chiK L.psiK
      hF L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1 L.hdelta
        L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table L.WUp) _ hPair

omit [CharP (ResidueField F) (Module.finrank F K)] in
/-- The boundary pencil has no vanishing numerator at a nonzero index. -/
private theorem lowGreen_boundaryNumerator_ne_zero :
    ∀ j : {j : ZMod p // j ≠ 0},
      ((j : ZMod p).val : ResidueField F) +
        wildOddBoundaryDZero generator base ≠ 0 := by
  intro j
  simpa only [lowGreen_boundaryGenerator, lowGreen_boundaryBase] using
...
include hepsilon in
/-- Put the upstairs factor in the homogeneous form of the boundary pencil. -/
private theorem lowGreen_boundaryUpperFactor_eq_scaled
    (hchar : residueCharacteristic F = p) :
    (lowSourceUpstairsPhase F K ht hres pi hpi hgen data L.chiK L.psiK
      hF L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1 L.hdelta
        L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table L.WUp
  ...
  have hDZero : wildOddBoundaryDZero generator base = (N).dZero := by
    simpa only [lowGreen_boundaryGenerator, lowGreen_boundaryBase,
      lowGreen_boundaryNormalization] using
      lowActual_boundaryDZero_eq F K ht hres pi hpi hgen data L.chiK
        L.psiK hF L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1
          L.hdelta L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table
            hepsilon hboundary hparity C hcharOdd
  have hnonzero := lowGreen_boundaryNumerator_ne_zero
    (ht := ht) (hres := hres) (pi := pi) (hpi := hpi) (hgen := hgen)
    (hF := hF) (L := L) hboundary hparity C hcharOdd
  let certificate := wildOdd_boundaryAffinePencilCertificate F p hchar C
    generator base hnonzero
  rw [lowGreen_boundaryUpperFactor (ht := ht) (hres := hres) (pi := pi)
    (hpi := hpi) (hgen := hgen) (hF := hF) (L := L) hboundary hepsilon
      hparity C hcharOdd hchar]
...
omit [CharP (ResidueField F) (Module.finrank F K)] in
private theorem lowGreen_boundaryBaseFactor :
    (lowSourceBasePhase F K ht hres pi hpi hgen data L.chiK L.psiK hF
      L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1 L.hdelta
        L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table).criticalFactor =
      wildOddLowerPairPhase C.lower (wildOddBasePair base) := by
  exact lowGreen_lowerFactor
    (lowSourceBasePhase F K ht hres pi hpi hgen data L.chiK L.psiK hF
      L.hminimal L.hchi L.hpsi L.hLow L.delta L.epsilon1 L.hdelta
        L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P L.table) C.lower
          C.lower_ne_one hcharOdd _
...
omit [CharP (ResidueField F) (Module.finrank F K)] in
/-- Remove the certified zero translation from each norm row. -/
private theorem lowGreen_boundaryNormFactor (j : OddNormIndex F K) :
    (lowSourceNormPhase F K ht hres pi hpi hgen data hF L.delta
      L.epsilon1 L.hdelta L.hT L.hgammaF L.P j).criticalFactor =
      wildOddLowerPairPhase C.lower
        (wildOddTauPowerPair generator ((j : ZMod p).val : ResidueField F)) := by
  apply lowGreen_lowerFactor
  have hPair := wildOdd_lowSourceNormCoefficients F K ht hres pi hpi
    hgen data hF L.delta L.epsilon1 L.hdelta L.hT L.hgammaF L.P j
      hparity C.lower C.lower_ne_one hcharOdd
  have hDelta := wildOdd_lowNormTranslationCoefficient_eq_zero F K ht
    hres pi hpi hgen data hF L.delta L.epsilon1 L.hdelta L.hT
      L.hgammaF L.P L.hodd j hparity C.lower C.lower_ne_one
  rw [hDelta] at hPair
  simpa only [lowGreen_boundaryGenerator,
    WildOddCoefficientPair.translateAffine, zero_mul, add_zero] using hPair

omit [CharP (ResidueField F) (Module.finrank F K)] in
/-- Remove the certified zero translation from each boundary twist row. -/
private theorem lowGreen_boundaryTwistFactor (j : OddNormIndex F K) :
    (lowSourceTwistPhase F K ht hres pi hpi hgen data L.chiK L.psiK hF
      ...
      wildOddLowerPairPhase C.lower
        (wildOddBoundaryPair generator base
          ((j : ZMod p).val : ResidueField F)) := by
  apply lowGreen_lowerFactor
  have hPair := wildOdd_lowBoundaryTwistCoefficients F K ht hres pi hpi
    hgen data L.chiK L.psiK hF L.hminimal L.hchi L.hpsi L.hLow L.delta
      L.epsilon1 L.hdelta L.hepsilon1 L.hT L.hgammaF L.hgammaK L.P
        L.table L.WTwist hboundary j hparity C.lower C.lower_ne_one hcharOdd
  have hDelta := wildOdd_lowTwistTranslationCoefficient_eq_zero F K ht
    hres pi hpi hgen data hF L.delta L.epsilon1 L.hdelta L.hT
      L.hgammaF L.P L.hodd j hparity C.lower C.lower_ne_one
  rw [hDelta] at hPair
  simpa only [lowGreen_boundaryGenerator, lowGreen_boundaryBase,
...
private theorem lowGreen_boundaryQuotient
  ...
  | succ s =>
    ...
    have hp2 : Module.finrank F K ≠ 2 := by
      ...
      rw [hp] at hodd
      norm_num at hodd
    simp_rw [lowGreen_boundaryNormFactor (ht := ht) (hres := hres)
      (pi := pi) (hpi := hpi) (hgen := hgen) (hF := hF) (L := L)
        hparity C hcharOdd,
      lowGreen_boundaryTwistFactor (ht := ht) (hres := hres)
        (pi := pi) (hpi := hpi) (hgen := hgen) (hF := hF) (L := L)
          hboundary hparity C hcharOdd]
    rw [lowGreen_boundaryUpperFactor_eq_scaled (ht := ht) (hres := hres)
      (pi := pi) (hpi := hpi) (hgen := hgen) (hF := hF) (L := L)
        hboundary hepsilon hparity C hcharOdd hchar,
      lowGreen_boundaryBaseFactor (ht := ht) (hres := hres) (pi := pi)
        (hpi := hpi) (hgen := hgen) (hF := hF) (L := L)
          hboundary hparity C hcharOdd]
    exact lowGreen_scaledBoundaryQuotient F (Module.finrank F K) hp2
      hchar C _ _ (lowGreen_boundaryNumerator_ne_zero (ht := ht)
        (hres := hres) (pi := pi) (hpi := hpi) (hgen := hgen) (hF := hF)
        (L := L) hboundary hparity C hcharOdd)
