...
private theorem degree_cast_ne_zero_of_dvd_card_sub_one
  ...
  have hcop : Nat.Coprime (Fintype.card k - 1) (Fintype.card k) :=
    (Nat.coprime_self_sub_left hqpos).mpr (Nat.coprime_one_left (Fintype.card k))
...
private def baseResidualAddChar
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (psiF : LocalAddCharData F) : FiniteAddChar (ResidueField F) :=
  residualAddChar F psiF (gammaF F K hres piK hpiK chiF psiF : Fˣ)
    (gammaF_order_one F K hres piK hpiK chiF hchiF psiF)

private theorem indexedNormCharacter_ne_one
    (j : ℕ) (hj : j ∈ Finset.Icc 1 (Module.finrank F K - 1)) :
    indexedNormCharacter F K ht hres piK hpiK hgen j ≠ 1 := by
  apply (ramifiedNormCharacterZModEquiv_ne_one_iff
    F K ht hres piK hpiK hgen _).2
  intro hone
  have hz : (j : ZMod (Module.finrank F K)) = 0 := by
    simpa using congrArg Multiplicative.toAdd hone
  have hjlt : j < Module.finrank F K := by
...
private theorem normGammaActual_eq_gammaF
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (psiF : LocalAddCharData F) (mu : NormCharacter F K) (hmu : mu ≠ 1) :
    (normGammaActual F K ht hres piK hpiK hgen psiF mu : Fˣ) =
      (gammaF F K hres piK hpiK chiF psiF : Fˣ) := by
  simp only [normGammaActual, gammaNorm, tameOddUniformizerGamma_coe, gammaF]
  rw [normData_nontrivial_conductor F K ht hres piK hpiK hgen mu hmu, hchiF]

private theorem twistGammaActual_eq_gammaF
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (hminimal : IsMinimalNormCharacterOrbitRepresentative F K chiF)
    (psiF : LocalAddCharData F) (mu : NormCharacter F K) :
    (twistGammaActual F K ht hres piK hpiK hgen chiF psiF mu : Fˣ) =
      (gammaF F K hres piK hpiK chiF psiF : Fˣ) := by
  simp only [twistGammaActual, gammaTwist, tameOddUniformizerGamma_coe, gammaF]
  rw [twistData_conductor_one F K ht hres piK hpiK hgen
    chiF hchiF hminimal mu, hchiF]

omit [PrimeCyclicExtension F K] in
private theorem normCharacter_gammaF_value
    (chiF : LocalQuasiCharData F) (psiF : LocalAddCharData F)
    (mu : NormCharacter F K) :
    mu.1 (gammaF F K hres piK hpiK chiF psiF : Fˣ) = 1 := by
  apply mu.eq_one_on_normRange F K
  refine ⟨(tameOddUpperUniformizer K piK hpiK) ^
      ((chiF.conductor : ℤ) + psiF.conductor), ?_⟩
  rw [map_zpow]
  rfl

omit [Module.Free F K] [PrimeCyclicExtension F K] in
/-- The conductor-one residue formula with the common lower denominator. -/
private theorem deltaFinite_commonGamma
    [Fintype (ResidueField F)]
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (psiF : LocalAddCharData F)
    (theta : LocalQuasiCharData F) (htheta : theta.conductor = 1)
    (Gamma : AdmissibleGamma F theta psiF)
    (hGamma : (Gamma : Fˣ) = (gammaF F K hres piK hpiK chiF psiF : Fˣ)) :
    deltaFinite theta psiF Gamma =
      -(theta.character (gammaF F K hres piK hpiK chiF psiF : Fˣ) : ℂ) *
        phase (langlandsGaussSum (residualMulChar F theta)
          (baseResidualAddChar F K hres piK hpiK chiF hchiF psiF)) := by
  cases Subsingleton.elim (inferInstance : Fintype (ResidueField F)) (residueFieldFintype F)
  simpa only [hGamma, baseResidualAddChar] using
    deltaFinite_conductor_one F theta htheta psiF Gamma

/-- Transport the upper Gauss sum and scalar to the lower residue field. -/
private theorem upper_delta_formula
    [Fintype (ResidueField F)]
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (hminimal : IsMinimalNormCharacterOrbitRepresentative F K chiF)
    (chiK : LocalQuasiCharData K)
    ...
    deltaFinite chiK psiK
        (gammaK F K ht hres piK hpiK hgen chiF chiK psiF psiK
          hminimal hcomp htrace (by omega)) =
      -((chiF.character (gammaF F K hres piK hpiK chiF psiF : Fˣ) : ℂ) ^
          Module.finrank F K) *
        phase ((residualMulChar F chiF
            ((Module.finrank F K : ResidueField F) ^ Module.finrank F K) : ℂ) *
          langlandsGaussSum (residualMulChar F chiF ^ Module.finrank F K)
            (baseResidualAddChar F K hres piK hpiK chiF hchiF psiF)) := by
  classical
  ...
  have hupperGauss :
    ...
    have hscale := langlandsGaussSum_mulShift_unit (chiBar ^ ell) psiBar ellUnit
    ...
    rfl
  change deltaFinite chiK psiK GammaK = _
  rw [deltaFinite_conductor_one K chiK hchiK psiK GammaK, hupperScalar, hupperGauss]

private theorem norm_delta_formula
    [Fintype (ResidueField F)]
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (psiF : LocalAddCharData F) (mu : NormCharacter F K) (hmu : mu ≠ 1) :
    deltaFinite (normData F K ht hres piK hpiK hgen mu) psiF
        (normGammaActual F K ht hres piK hpiK hgen psiF mu) =
      -phase (langlandsGaussSum
        (residualMulChar F (normData F K ht hres piK hpiK hgen mu))
        (baseResidualAddChar F K hres piK hpiK chiF hchiF psiF)) := by
  rw [deltaFinite_commonGamma F K hres piK hpiK chiF hchiF psiF _
    (normData_nontrivial_conductor F K ht hres piK hpiK hgen mu hmu) _
    (normGammaActual_eq_gammaF F K ht hres piK hpiK hgen chiF hchiF psiF mu hmu)]
  rw [normData_character, normCharacter_gammaF_value F K hres piK hpiK chiF psiF mu]
  simp

/-- The same scalar and additive character serve every twist, including the identity. -/
private theorem twist_delta_formula
    [Fintype (ResidueField F)]
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (hminimal : IsMinimalNormCharacterOrbitRepresentative F K chiF)
    (psiF : LocalAddCharData F) (mu : NormCharacter F K) :
    deltaFinite (twistData F K ht hres piK hpiK hgen chiF mu) psiF
        (twistGammaActual F K ht hres piK hpiK hgen chiF psiF mu) =
      -(chiF.character (gammaF F K hres piK hpiK chiF psiF : Fˣ) : ℂ) *
        phase (langlandsGaussSum
          (residualMulChar F (twistData F K ht hres piK hpiK hgen chiF mu))
          (baseResidualAddChar F K hres piK hpiK chiF hchiF psiF)) := by
  rw [deltaFinite_commonGamma F K hres piK hpiK chiF hchiF psiF _
    (twistData_conductor_one F K ht hres piK hpiK hgen chiF hchiF hminimal mu) _
    (twistGammaActual_eq_gammaF F K ht hres piK hpiK hgen
      chiF hchiF hminimal psiF mu)]
  rw [twistData_character, ContinuousQuasiChar.mul_apply,
    normCharacter_gammaF_value F K hres piK hpiK chiF psiF mu, one_mul]

/-- Package the actual endpoints and their residue formulas for product assembly. -/
private def productDeltaData
    [Fintype (ResidueField F)]
    (chiF : LocalQuasiCharData F) (hchiF : chiF.conductor = 1)
    (hminimal : IsMinimalNormCharacterOrbitRepresentative F K chiF)
    (chiK : LocalQuasiCharData K)
    ...
    (C : ResidualCertificates F K ht hres piK hpiK hgen
      chiF hchiF hminimal chiK hcomp psiF psiK htrace) :
    TameOddConductorOneDeltaData (Module.finrank F K) C.eta (residualMulChar F chiF)
      (baseResidualAddChar F K hres piK hpiK chiF hchiF psiF) where
  scalar := (chiF.character (gammaF F K hres piK hpiK chiF psiF : Fˣ) : ℂ)
  upperDelta := deltaFinite chiK psiK
    (gammaK F K ht hres piK hpiK hgen chiF chiK psiF psiK
      hminimal hcomp htrace (by omega))
  normDelta := fun j => deltaFinite
    (normData F K ht hres piK hpiK hgen
      (indexedNormCharacter F K ht hres piK hpiK hgen j)) psiF
    (normGammaActual F K ht hres piK hpiK hgen psiF
      (indexedNormCharacter F K ht hres piK hpiK hgen j))
  baseDelta := deltaFinite (twistData F K ht hres piK hpiK hgen chiF 1) psiF
    (twistGammaActual F K ht hres piK hpiK hgen chiF psiF 1)
  twistDelta := fun j => deltaFinite
    (twistData F K ht hres piK hpiK hgen chiF
      (indexedNormCharacter F K ht hres piK hpiK hgen j)) psiF
    (twistGammaActual F K ht hres piK hpiK hgen chiF psiF
      (indexedNormCharacter F K ht hres piK hpiK hgen j))
  upper_formula := upper_delta_formula F K ht hres piK hpiK hgen
    chiF hchiF hminimal chiK hcomp psiF psiK htrace C
  norm_formula := by
    intro j hj
    rw [norm_delta_formula F K ht hres piK hpiK hgen chiF hchiF psiF _
      (indexedNormCharacter_ne_one F K ht hres piK hpiK hgen j hj),
      indexedNormCharacter, C.norm_rows j hj]
  base_formula := by
    rw [twist_delta_formula F K ht hres piK hpiK hgen chiF hchiF hminimal psiF 1]
    have hresidual :
        residualMulChar F (twistData F K ht hres piK hpiK hgen chiF 1) =
          residualMulChar F chiF := by
  ...
  twist_formula := by
    intro j hj
    let mu := indexedNormCharacter F K ht hres piK hpiK hgen j
    rw [twist_delta_formula F K ht hres piK hpiK hgen chiF hchiF hminimal psiF]
    have hresidual :
        residualMulChar F (twistData F K ht hres piK hpiK hgen chiF mu) =
          residualMulChar F chiF * C.eta ^ j := by
      calc
        _ = residualMulChar F (normData F K ht hres piK hpiK hgen mu) *
              residualMulChar F chiF := by
          apply residualMulChar_eq_mul_of_character_eq
          rw [twistData_character, normData_character]
        _ = C.eta ^ j * residualMulChar F chiF := by
          rw [show residualMulChar F (normData F K ht hres piK hpiK hgen mu) =
            C.eta ^ j from C.norm_rows j hj]
        _ = residualMulChar F chiF * C.eta ^ j := mul_comm _ _
    rw [hresidual]
...
theorem full_product
  ...
  let chiBar := residualMulChar F chiF
  let psiBar := baseResidualAddChar F K hres piK hpiK chiF hchiF psiF
  let D := productDeltaData F K ht hres piK hpiK hgen
    chiF hchiF hminimal chiK hcomp psiF psiK htrace C
  have hdiv : Module.finrank F K ∣ Fintype.card (ResidueField F) - 1 := by
    rw [← C.eta_order]
    exact MulChar.orderOf_dvd_card_sub_one (ResidueField F) C.eta
  apply D.full_normCharacter_product F K ht hres piK hpiK hgen
    C.eta chiBar psiBar hdiv C.eta_order
    (residualAddChar_ne_one F psiF _
      (gammaF_order_one F K hres piK hpiK chiF hchiF psiF))
    (deltaFinite chiK psiK
      (gammaK F K ht hres piK hpiK hgen chiF chiK psiF psiK
        hminimal hcomp htrace (by omega)))
