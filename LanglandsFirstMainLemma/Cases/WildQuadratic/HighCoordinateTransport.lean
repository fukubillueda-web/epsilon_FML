...
section CoordinateTransportStages
...
omit [Finite (NormCharacter F K)] [Fintype (ResidueField F)] [CharP (ResidueField F) 2]
  [Fintype (ResidueField K)] [CharP (ResidueField K) 2] in
/-- The conductor arithmetic shared by both upper-coordinate branches. -/
private theorem wildQuadraticPreparation_coordinateTransport_depthAndParity
    {t : ℕ}
    (ht : PrimeCyclicExtension.IsLowerBreak F K t)
    (hres : residueDegree F K = 1)
    (pi : ringOfIntegers K)
    ...
    chiK.conductor / 2 =
        wildQuadraticHighShift t chiF.conductor +
          wildQuadraticHighNormPrecision t ∧
...
/-- Build the strict-high upper certificate when the base conductor is even. -/
private theorem
    wildQuadraticPreparation_coordinateTransport_aboveMEvenTOdd
    {t : ℕ}
    (ht : PrimeCyclicExtension.IsLowerBreak F K t)
    (hres : residueDegree F K = 1)
    (pi : ringOfIntegers K)
    ...
    (hcorrectionC : V.correction = C)
    (htauData : V.tauData =
      wildQuadraticHighTauData F K ht hres pi hpi hgen tau (by assumption))
    ...
    (coordinates : WildQuadraticCommonCoordinateCompatibility V data
      phaseData assembly) :
    ∀ (tauW : QuotientDerivedNormalizedCriticalFunction F)
      (_hI : V.criticalInputs = .aboveMEvenTOdd tauW)
      (q : CharTwoRefinement (ResidueField F)),
      Nonempty (ActualUpperCoordinateCertificate F K V.correction q
        V.criticalInputs V.stationaryPairs V.tauData V.chiFData V.psiF) := by
  classical
  ...
  let extensionDepth := chiK.conductor / 2
  have hdK : extensionDepth = v + r := by
    simpa only [extensionDepth, v, r] using hdepthAndParity.1
  have hepsilonK :
      chiK.conductor % 2 = (t + 1) % 2 :=
    hdepthAndParity.2
  have hu : ord K (C.u : K) =
  ...
  have upperDepth : upper.d = extensionDepth := by
    ...
    dsimp only [extensionDepth]
    omega
  have upperDelta : ord K (upper.delta : K) =
      ((extensionDepth : ℤ) : WithTop ℤ) := by
...
set_option maxHeartbeats 4000000 in
/-- Build the strict-high upper certificate when the base conductor is odd. -/
private theorem
    wildQuadraticPreparation_coordinateTransport_aboveMOddTOdd
    {t : ℕ}
    (ht : PrimeCyclicExtension.IsLowerBreak F K t)
    (hres : residueDegree F K = 1)
    (pi : ringOfIntegers K)
    ...
    (rows : WildQuadraticActualPhaseRows V data phaseData assembly) :
    ∀ (tauW chiW : QuotientDerivedNormalizedCriticalFunction F)
      (_hI : V.criticalInputs = .aboveMOddTOdd tauW chiW)
      (q : CharTwoRefinement (ResidueField F)),
      Nonempty (ActualUpperCoordinateCertificate F K V.correction q
        V.criticalInputs V.stationaryPairs V.tauData V.chiFData V.psiF) := by
  classical
...
set_option maxHeartbeats 4000000 in
/-- Assemble the coordinate transport once both source-bearing branches exist. -/
private theorem
    wildQuadraticPreparation_coordinateTransport_ofUpperBranches
    {t : ℕ}
    (chiF : LocalQuasiCharData F) (chiK : LocalQuasiCharData K)
    {dK epsilonK : ℕ}
    (hK : IsStationaryConductorDecomposition chiK.conductor dK epsilonK)
    ...
    (V : WildQuadraticCommonCoefficientView F K t chiF.conductor)
    (hchiData : V.chiFData = chiF)
    (hconductorParity : chiK.conductor % 2 = (t + 1) % 2)
    {data : FirstMainComputationalData F K V.chiFData.character
      V.psiF.character}
    {phaseData : FirstMainPhaseData F K V.chiFData.character
      V.psiF.character data}
  ...
  have hinputAbove : V.criticalInputs.row.range = .above := by
    have habove : t < chiF.conductor - 1 := by omega
    rw [V.criticalInputs_row,
  ...
  have hExtensionEven (hTEven : Even (t + 1)) :
    ...
    have hTmod : (t + 1) % 2 = 0 := by
      obtain ⟨j, hj⟩ := hTEven
      omega
    have hKmod : chiK.conductor % 2 = 0 :=
      hconductorParity.trans hTmod
...
noncomputable def coordinateTransport
  ...
  have hdepthAndParity :=
    wildQuadraticPreparation_coordinateTransport_depthAndParity
      F K ht hres pi hpi hgen hdegree htpos chiF chiK hF hK hminimal
        hhigh hchi
  have hAboveEven :=
    wildQuadraticPreparation_coordinateTransport_aboveMEvenTOdd
      F K ht hres pi hpi hgen hdegree htpos chiF chiK psiF hK hchi tau
        htau C hstrict V hcorrectionC htauData hchiData hpsiData
          hdepthAndParity rows coordinates
  have hAboveOdd :=
    wildQuadraticPreparation_coordinateTransport_aboveMOddTOdd
      F K ht hres pi hpi hgen hdegree htpos chiF chiK hK hchi tau htau
        C hstrict V hcorrectionC htauData hchiData hdepthAndParity rows
  exact
    wildQuadraticPreparation_coordinateTransport_ofUpperBranches
      F K chiF chiK hK hchi hstrict V hchiData hdepthAndParity.2 rows
        hAboveEven hAboveOdd

end WildQuadraticHighParameterData

section HighContinuationStages
...
/-- Conductor and row identities shared by every strict-high continuation
stage. -/
private theorem wildQuadraticPreparation_strictHighContinuation_rowFacts
    (coordinates : WildQuadraticCommonCoordinateCompatibility V data
      phaseData assembly)
    (hstrict : t + 1 < m) :
    V.chiFData.conductor = m ∧
      V.criticalInputs.row.range = .above ∧
      V.criticalInputs.row.TParity =
        WildQuadraticConductorParity.ofConductor (t + 1) ∧
...
/-- The retained tau row has the neutral source phase when its conductor is
even. -/
private theorem wildQuadraticPreparation_tauSourcePhase_of_even
    (rows : WildQuadraticActualPhaseRows V data phaseData assembly)
    (coordinates : WildQuadraticCommonCoordinateCompatibility V data
      phaseData assembly)
    (htauConductor : V.tauData.conductor = t + 1)
...
/-- The retained base row has the neutral source phase when its conductor is
even. -/
private theorem wildQuadraticPreparation_baseSourcePhase_of_even
    (rows : WildQuadraticActualPhaseRows V data phaseData assembly)
    (coordinates : WildQuadraticCommonCoordinateCompatibility V data
      phaseData assembly)
    (hbaseConductor : V.chiFData.conductor = m)
    (hmEven : Even m) :
...
/-- In a strict-high row, absence of a lower-product coefficient forces the
actual twist phase to be neutral. -/
private theorem wildQuadraticPreparation_strictHighProductPhase_of_none
    (rows : WildQuadraticActualPhaseRows V data phaseData assembly)
    (hstrict : t + 1 < m)
    (htwistConductor :
      (data.twistData assembly.indexing.tau).conductor = max m (t + 1))
    (hinputAbove : V.criticalInputs.row.range = .above)
    (hmParityOfRow :
      V.criticalInputs.row.mParity =
        WildQuadraticConductorParity.ofConductor m) :
    ∀ q : CharTwoRefinement (ResidueField F),
...
/-- Identify the retained tau and base phases with the sources encoded by
the strict-high coefficient row. -/
private theorem wildQuadraticPreparation_strictHighSourcePhases
    (rows : WildQuadraticActualPhaseRows V data phaseData assembly)
    (coordinates : WildQuadraticCommonCoordinateCompatibility V data
      phaseData assembly)
    (htauConductor : V.tauData.conductor = t + 1)
  ...
  have hTauEvenPhase (hTEven : Even (t + 1)) :
      rows.tau.criticalFactor = quotientSourcePhase F none :=
    wildQuadraticPreparation_tauSourcePhase_of_even rows coordinates
      htauConductor hTEven
  have hBaseEvenPhase (hmEven : Even m) :
      rows.base.criticalFactor = quotientSourcePhase F none :=
    wildQuadraticPreparation_baseSourcePhase_of_even rows coordinates
      hbaseConductor hmEven
  cases hI : V.criticalInputs with
  | belowMEvenTEven | belowMEvenTOdd | belowMOddTEven | belowMOddTOdd |
      boundaryEven | boundaryOdd =>
      have hbad := hinputAbove
...
/-- Assemble the final q-polymorphic continuation from the product callback
and the three phase identities. -/
private theorem
    wildQuadraticPreparation_strictHighContinuation_of_phases
    (rows : WildQuadraticActualPhaseRows V data phaseData assembly)
    (coordinate : WildQuadraticPositivePolarCoordinateTransport F K V rows)
    (hproduct : ∀ (q : CharTwoRefinement (ResidueField F))
      {lambda : ResidueField F},
      (V.criticalInputs.toCoefficientData q).affineCoefficient .tauChiF =
          some lambda →
    ...
    (hproductNone : ∀ q : CharTwoRefinement (ResidueField F),
      (V.criticalInputs.toCoefficientData q).affineCoefficient .tauChiF = none →
  ...
  exact
    { coordinate := coordinate
      lowerProduct := by
        ...
        exact ⟨C0⟩
      tau_phase := fun _q ↦ hsourcePhases.1
      base_phase := fun _q ↦ hsourcePhases.2
      twist_phase := by
        intro q
        cases haff :
...
noncomputable def wildQuadraticHighContinuationFromCoordinate
    ...
    ∀ source : V.criticalInputs.PositivePolarSource,
      WildQuadraticPositivePolarContinuation F K V source rows := by
  obtain ⟨hbaseConductor, hinputAbove, hTParityOfRow,
      hmParityOfRow⟩ :=
    wildQuadraticPreparation_strictHighContinuation_rowFacts coordinates
      hstrict
  ...
  have hproductNone : ∀ q : CharTwoRefinement (ResidueField F),
      (V.criticalInputs.toCoefficientData q).affineCoefficient .tauChiF = none →
        rows.twist.criticalFactor = 1 :=
    wildQuadraticPreparation_strictHighProductPhase_of_none rows hstrict
      htwistConductor hinputAbove hmParityOfRow
  have hsourcePhases :
      rows.tau.criticalFactor =
          quotientSourcePhase F V.criticalInputs.tauSource ∧
        rows.base.criticalFactor =
          quotientSourcePhase F V.criticalInputs.chiFSource :=
    wildQuadraticPreparation_strictHighSourcePhases rows coordinates
      htauConductor hbaseConductor hinputAbove hTParityOfRow hmParityOfRow
  exact
    wildQuadraticPreparation_strictHighContinuation_of_phases rows coordinate
      hproduct hproductNone hsourcePhases
