...
private noncomputable def wildQuadraticBoundaryCompletion
  ...
  rcases hpairs with
    ⟨⟨htauGamma, htauBeta⟩, ⟨hchiGamma, hchiBeta⟩⟩
  rcases hlocal with
    ⟨⟨htauLocalChi, htauLocalPsi⟩, ⟨hchiLocalChi, hchiLocalPsi⟩⟩
  ...
  have hWtauConductor : Wtau.chi.conductor = t + 1 := by
    rw [htauLocalChi, ← coordinates.tauData_eq, htauConductor]
  ...
  have htheta : Wtau.chi.character = assembly.indexing.tau.1 := by
    rw [htauLocalChi, ← coordinates.tauData_eq,
  ...
  let hcore := boundaryCore pi hpi hgen view.correction rho hden Wtau Wchi
    BC assembly.indexing.tau htheta hT h2 assembly.A view.correction.X
      htauRatio.symm hX
  let hy := Classical.choose hcore
  let hxData := Classical.choose_spec hcore
  let hx := Classical.choose hxData
  have hcoreSpec := Classical.choose_spec hxData
  rcases hcoreSpec with ⟨hyred, hxred, helemW⟩
  ...
  let chiCorrection := wildQuadraticCorrectionForLocalData Wchi q
    view.chiFData view.psiF hchiLocalChi hchiLocalPsi view.correction.x hx
      hchiOne
  let tauCorrection := wildQuadraticCorrectionForLocalData Wtau q
    view.tauData view.psiF htauLocalChi htauLocalPsi view.correction.y hy
      htauOne
  ...
  have htauSpec := wildQuadraticCorrectionForLocalData_spec Wtau q
    view.tauData view.psiF htauLocalChi htauLocalPsi view.correction.y hy
      htauOne
  rcases hchiSpec with
    ⟨hchiCorrection, hchiCorrectionGamma, hchiCorrectionBeta,
      hchiCorrectionClass, hchiCorrectionAffine⟩
  rcases htauSpec with
    ⟨htauCorrection, htauCorrectionGamma, htauCorrectionBeta,
      htauCorrectionClass, htauCorrectionAffine⟩
  have hchiRatio : (Wchi.beta : F) / ((Wchi.Gamma : Fˣ) : F) =
      assembly.A * assembly.n := by
    rw [hchiGamma, hchiBeta]
  ...
  let refinements : WildQuadraticRefinementAssembly view data phaseData
    ...
    { chiCorrection := some chiCorrection
      ...
      chi_selection := by
        ...
        congr 1
        exact hchiCorrectionClass.trans hxred
      tau_selection := by
        simp only [Option.map_some]
        rw [hcoeff, WildQuadraticBoundaryData.coefficientData_tauCorrectionClass]
        congr 1
        exact htauCorrectionClass.trans hyred
      chi_absent := by simp
      tau_absent := by simp
      chi_present := by
        intro C hC
        ...
        subst C
        refine ⟨hchiCorrection.trans coordinates.x_eq.symm,
          hchiCorrectionGamma.trans hchiGamma,
          hchiCorrectionBeta.trans hchiBeta, ?_, ?_, ?_⟩
        · rw [hchiCorrectionBeta, hchiCorrectionGamma]
          exact hchiRatio
        · rw [hcoeff]
          change some boundary.gamma = some chiCorrection.affineCoefficient
          congr 1
          simpa only [boundary, gamma] using hchiCorrectionAffine.symm
        · rw [hcoeff]
          change some boundary.B = some chiCorrection.normalizedClass
          congr 1
          exact (hchiCorrectionClass.trans hxred).symm
      tau_present := by
        intro C hC
        have hEq : tauCorrection = C := Option.some.inj hC
        subst C
        refine ⟨htauCorrection.trans coordinates.y_eq.symm,
          htauCorrectionGamma.trans htauGamma,
          htauCorrectionBeta.trans htauBeta, ?_, ?_, ?_⟩
        · rw [htauCorrectionBeta, htauCorrectionGamma]
          exact htauRatio
        · rw [hcoeff]
          change some boundary.gammaZero = some tauCorrection.affineCoefficient
          congr 1
          simpa only [boundary, gammaZero] using htauCorrectionAffine.symm
        · rw [hcoeff]
          change some boundary.C = some tauCorrection.normalizedClass
          congr 1
          exact (htauCorrectionClass.trans hyred).symm }
  have helem : (view.psiF.character
      (-assembly.A * view.correction.X) : ℂ) =
      absoluteTraceChar (ResidueField F) boundary.C := by
    rw [htauLocalPsi] at helemW
...
private noncomputable def wildQuadraticStrictHighOddCompletion
  ...
  rcases hCspec with
    ⟨hCCorrection, hCGamma, hCBeta, hCNormalizedClass, hCAffine⟩
  have hCClass : Cchi.normalizedClass = 1 :=
    hCNormalizedClass.trans hxred
  have hchiClass :
  ...
  let refinements : WildQuadraticRefinementAssembly view data phaseData
    ...
    { chiCorrection := some Cchi
      ...
      chi_present := by
        ...
        subst C
        exact ⟨hCCorrection, hCGamma.trans hWPair.1,
          hCBeta.trans hWPair.2, hratio, by
            rw [hchiAffine, hCAffine], by
